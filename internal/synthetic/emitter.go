...
func (e *Emitter) Emit(ctx context.Context, traceID, parentSpanID, name, service string, attributes map[string]string) (string, error) {
	...
	span := map[string]any{"traceId": traceID, "spanId": spanID, "name": name, "attributes": items}
	if parentSpanID != "" {
