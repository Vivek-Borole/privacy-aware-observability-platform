...
type Result = { traceId: string; spans?: Span[] | null };
...
function App() {
  ...
  async function lookup(event: React.FormEvent) {
    ...
    try {
      ...
      const safeResult = { ...next, spans: Array.isArray(next.spans) ? next.spans : [] };
      setResult(safeResult);
      setStatus(safeResult.spans.length ? `${safeResult.spans.length} sanitized span(s) found.` : "No visible spans for this tenant and trace.");
  ...
  async function loadDependencies() {
    ...
    try {
      ...
      const safeDependencies = { ...next, dependencies: Array.isArray(next.dependencies) ? next.dependencies : [] };
      setDependencies(safeDependencies);
      setStatus(safeDependencies.dependencies.length ? "Derived service topology loaded. Edges contain no raw telemetry." : "No service-to-service edges are visible for this tenant yet.");
  ...
  async function loadAudit() {
    ...
    try {
      ...
      const safeAudit = { ...next, events: Array.isArray(next.events) ? next.events : [] };
      setAudit(safeAudit);
      setStatus(safeAudit.events.length ? "Safe audit timeline loaded." : "No tenant audit events are visible yet.");
  ...
  async function loadSampling() {
    ...
    try {
      ...
      const safeSampling = { ...next, decisions: Array.isArray(next.decisions) ? next.decisions : [] };
      setSampling(safeSampling);
      setStatus(safeSampling.decisions.length ? "Explicit sampling decisions loaded." : "No tail-sampling decisions are visible for this tenant yet.");
